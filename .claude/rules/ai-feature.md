...
- Generation lives in `server/utils/aiGenerate.ts`: `generateText` + `Output.object` (AI SDK 7), with the system prompt in `instructions`. AI SDK 7 throws `InvalidPromptError` for system messages inside `messages`. The route only validates input, rate-limits, and picks the provider.
